...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes(String jsonResponseString) {
        ...
        //Make sure the JSON object is not an empty string
        ...
        try {
            ...
            JSONArray earthquakesArray = rootObject.optJSONArray("features");

            //for each earthquake im the earthquakesArray, create an Earthquake object
            for(int i=0; i < earthquakesArray.length(); i++)
            {
                ...
                String detailedUrl = earthquakeProperties.getString("url");



                //add it to the array that will be returned later
                earthquakes.add(new Earthquake(magnitude, place, date, detailedUrl));
            }
